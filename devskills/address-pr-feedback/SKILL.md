...
- Match the branch's existing commit message style (prefixes, mood, length, ticket references, punctuation). Infer it from `git log`; never impose an external convention such as Conventional Commits if the branch doesn't already use it.
...
Look for prefix conventions (Conventional Commits, ticket IDs, all-caps type tags, etc.), mood (imperative vs. descriptive), and typical length. Reuse that exact style for any new commit created in this workflow. Do not introduce a different convention than what the branch already uses.
...
If a fixup does not apply cleanly during the rebase, stop, resolve the conflict in place, re-run the narrowest relevant check, and continue the rebase.
...
Before pushing or touching GitHub in any way, present a complete preview and get explicit approval via the user-question tool.
...
History was rewritten via interactive rebase, so pushing requires a force push. Warn about the implications for anyone else who has pulled this branch.
...
- New commits (if any) match the branch's existing message style exactly.
...
- Do not impose a commit message convention the branch doesn't already use.
